...
miRNAlist<-unique(targetscan[which(targetscan[,4]=="10090"),5])

#for (miRNA in miRNAlist){

miRNA="mmu-miR-17-5p"
target<-targetscan[(targetscan[,5]==miRNA) & targetscan[,4]=="10090",c(2,12)]
names(target)<-c("gene","percentile")
target$gene<-str_to_title(target$gene)
toptarget<-target[target$percentile>95,1]
...
meandif<-mean(toptargetDE)-mean(randomDE)
mediandif<-median(toptargetDE)-median(randomDE)
...
toptargetup<-toptargetDE[toptargetDE$logFC>0,]
dim(toptargetup)
toptargetdown<-toptargetDE[toptargetDE$logFC<0,]
dim(toptargetdown)

p<-t.test(randomDE$logFC,toptargetDE$logFC)$p.value
p<-paste(miRNA,p,meandif,mediandif,sep=",")
print(p)

plotDE<-rbind(targetDE,randomDE,toptargetDE)
plotDE<-rbind(randomDE,toptargetDE)
